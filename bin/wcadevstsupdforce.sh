...
mysql -u sam -p"$mysqlpw" wca_stats -e "UPDATE last_updated SET notes = 'Force update started; developer database and wca_stats now being updated --- (${URLStamp} vs ${localStamp})' WHERE query = 'wcadevstsupd.sh'"
## post to discord that it's started
~/.mysqlpw/discord-notify.sh "Forced update started; developer database is now being downloaded"
## start timer
start_timer=$(date +%s)
