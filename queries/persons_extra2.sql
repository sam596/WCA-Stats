...
CREATE TABLE persons_extra	
		...
		LEFT JOIN 
			(SELECT personId, 
				...
				COUNT(DISTINCT c.continentId) `continents`
			FROM result_dates a
			JOIN wca_dev.competitions b
			ON a.competitionId = b.id
			JOIN wca_dev.countries c
			ON b.countryId = c.id
			GROUP BY personId) c
		ON a.id = c.personId
		LEFT JOIN
