...
(PRIMARY KEY (id))
    SELECT
      ...
      @m := (CASE WHEN c.eventsSucceeded <> 17 THEN 0 ELSE 1 + (CASE WHEN c.speedsolvingEventsAverage = 12 THEN 1 ELSE 0 END) + (CASE WHEN c.bldfmcEventsAverage = 4 THEN 1 ELSE 0 END) + (CASE WHEN l.wcPodiums > 0 THEN 1 ELSE 0 END) + (CASE WHEN c.WRs > 0 THEN 1 ELSE 0 END) + (CASE WHEN c.currentEventsWon = 17 THEN 1 ELSE 0 END) END) mhelp,
