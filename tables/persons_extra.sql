...
CREATE TABLE pe_all_attempts
(KEY pe_attempts_person (personId))
SELECT personId, 
    COUNT(CASE WHEN result > 0 THEN 1 END) completedSolves, 
    COUNT(CASE WHEN result = -1 THEN 1 END) DNFs
  FROM all_attempts 
  GROUP BY personId;
  
DROP TABLE IF EXISTS pe_ranks_all;
CREATE TABLE pe_ranks_all
